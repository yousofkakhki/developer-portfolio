...
const SingleProject = ({ project }) => {
  ...
  return (
    <div className='group w-full h-fit flex flex-col items-center justify-center relative cursor-text overflow-hidden px-3 md:px-8 py-[1.4rem] bg-[linear-gradient(135deg,#101B27_0%,#071018_100%)] shadow-2xl rounded-lg border border-[#4C5560]'
    >
      <div className="absolute left-0 top-0 flex justify-center opacity-40">
        <svg width="1170" height="403" viewBox="0 0 1170 403" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fillRule="evenodd" clipRule="evenodd" d="M1170 57.3509H0V56.5132H1170V57.3509Z" fill="#16F2B3" fillOpacity="0.18"></path>
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        <h2 className='font-display text-[#E6E8F0] not-italic font-medium text-[1.525rem] leading-[110%] text-center capitalize'>
          {name}
        </h2>
        <div className="p-6">
          <Image
        ...
        <div className="flex items-center justify-between w-full">
          <Link
            href={demo}
            target='_blank'
            className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#16F2B3] text-[#16F2B3] transition-all duration-300 hover:bg-[#0A7B68] hover:text-[#F3F6F8] hover:border-[#16F2B3] hover:scale-110 decoration-clone cursor-pointer no-underline delay-[0.3s]">
            <FaPlay />
          </Link>
          ...
          <Link
            href={code}
            target='_blank'
            className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-[#16F2B3] text-[#16F2B3] transition-all duration-300 hover:bg-[#0A7B68] hover:text-[#F3F6F8] hover:border-[#16F2B3] hover:scale-110 cursor-pointer no-underline  delay-[0.3s] group-hover:translate-x-[-140px]">
            <FaCode />
          </Link>
        </div>
      </div>
      <p className="absolute w-[90%] md:w-[85%] md:min-h-[150px] translate-x-[-110%] transition-transform duration-[0.9s] p-6 leading-[110%] rounded-[0_20px_20px_0] left-0 top-0 bg-[#071018] text-[#E6E8F0] border border-[#4C5560] translate-y-[25%] md:translate-y-[50%] group-hover:translate-x-[-2%] text-xs md:text-sm">
        {description}
      </p>
      <div className='group-hover:translate-x-0 absolute w-[140px] text-[0.8rem] flex justify-center gap-2 flex-col translate-x-full transition-transform duration-[0.5s] delay-[0.3s] p-[0.825rem] rounded-[10px_0_0_10px] right-0 bottom-4 bg-[#101B27] text-[#E6E8F0] border border-[#4C5560]'>
        {tags.map((tag, id) => (
